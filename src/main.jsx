...
import {
  ...
  Upload,
  Users,
  Loader2 // Tambahan ikon loading berputar
} from "lucide-react";
import { API_URL, APP_NAME } from "./config";
import "./styles.css";

// Data demo lokal jika API_URL kosong
const demo = {
  students: [
...
// Fungsi Jembatan API yang Sudah Diperbaiki (Anti-Bengong & Anti-CORS)
async function api(action, payload = {}) {
  ...
  try {
    const r = await fetch(API_URL, {
      method: "POST",
      // PENTING: Kita hapus headers "Content-Type" agar Google tidak memblokirnya (CORS)
...
function localAction(action, b) {
  if (action === "login") {
    ...
    return u
      ? { ok: true, user: { id: u.Id, name: u.Name, role: u.Role || "student", grade: u.Grade } }
      : { ok: false, error: "Nama atau PIN salah. Periksa kembali huruf besar/kecilnya." };
  }
...
function App() {
  ...
  const [isLoggingIn, setIsLoggingIn] = useState(false); // State baru untuk loading
  ...
  const login = async (name, pin) => {
    ...
    setIsLoggingIn(true);
    setNotice(""); // Kosongkan error sebelumnya
  ...
  return (
    <div className="overlay" onMouseDown={!loading ? onClose : undefined}>
      <div className="login-modal" onMouseDown={(e) => e.stopPropagation()}>
        ...
        {/* Pesan Error yang lebih mencolok */}
        {notice && (
        ...
        >
          {loading ? (
            <>
              {/* Animasi Berputar */}
              <Loader2 className="lucide lucide-loader-2" size={18} style={{ animation: "spin 1s linear infinite" }} /> 
              Memeriksa...
            </>
          ) : (
        ...
        {!loading && (
          <button className="link-button" onClick={onClose}>Kembali</button>
        )}
        
        {/* CSS keyframe untuk putaran ditaruh langsung di sini agar aman */}
        <style>{`
...
function Dashboard({ user, data, go }) {
  ...
  return (
    <>
      ...
      <div className="stats">
        {(user.role === "teacher"
          ? [[Users, data.students.length, "Murid", "students"], [CalendarCheck2, present, "Hadir", "attendance"], [BookOpen, data.materials.length, "Materi", "materials"], [ClipboardCheck, data.assignments.length, "Tugas", "assignments"]]
          : [[CalendarCheck2, present, "Hadir", "attendance"], [BookOpen, data.materials.length, "Materi", "materials"], [ClipboardCheck, data.assignments.length, "Tugas", "assignments"], [Megaphone, data.announcements.length, "Kabar", "announcements"]]
        ).map(([I, v, l, p]) => (
