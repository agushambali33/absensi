  ...
  materials: [
    {
      ...
      Date: "2026-08-31",
      Content: "Latihan perkalian 2–10. Kerjakan dengan santai dan teliti.",
    },
    {
      Title: "Mengenal Pecahan",
      Date: "2026-08-29",
      Content: "Pahami pembilang dan penyebut melalui contoh sehari-hari.",
    },
  ],
  assignments: [
    {
      ...
      Due: "2026-09-02",
      Instructions: "Kerjakan 10 soal perkalian di buku.",
    },
  ],
  announcements: [
    {
      Title: "Selamat datang di Ruang Belajar",
      Content: "Jangan lupa isi absensi sebelum belajar ya 🌷",
      Date: "2026-08-31",
    },
  ],
  submissions: [],
...
function localAction(action, b) {
  if (action === "login") {
    const users = [
      ...demo.students,
      { Id: "teacher", Name: "Guru", PIN: "1234", Role: "teacher" },
    ];
    const u = users.find(
      (x) =>
        x.PIN === b.pin &&
    ...
    return u
      ? {
          ok: true,
          user: { id: u.Id, name: u.Name, role: u.Role || "student", grade: u.Grade },
        }
      : {
          ok: false,
          error:
            "Nama atau PIN salah. Mode demo: Guru 1234, siswa Alya 1111, Raka 2222, dsb.",
        };
  }
  if (action === "data") return { ok: true, ...demo };
  if (action === "attendance") {
    demo.attendance.unshift({
      ...b,
  ...
  if (["material", "assignment", "announcement", "submission"].includes(action)) {
    const key = {
      material: "materials",
...
function App() {
  ...
  return (
    <Shell user={user} page={page} setPage={setPage} logout={logout}>
      {page === "dashboard" ? (
        <Dashboard user={user} data={data} go={setPage} />
      ) : page === "attendance" ? (
    ...
    <div className="landing">
      ...
      <main className="hero">
        <div className="hero-copy">
          ...
          <p>
            Ruang sederhana untuk absensi, materi, tugas, dan kabar belajar. Dibuat
            hangat untuk guru dan murid.
          </p>
          <button className="hero-button" onClick={onLogin}>
            Masuk ke Ruang Belajar <ChevronRight size={18} />
          </button>
          <div className="hero-note">
            <Heart size={14} /> Dibuat untuk kelas kecil, dengan perhatian yang besar.
          </div>
        </div>
        <div className="flower-art">
          <div className="sun"></div>
          <div className="stem s1"></div>
          <div className="stem s2"></div>
          <div className="stem s3"></div>
      ...
      <section className="features">
        <div>
          <CalendarCheck2 />
          <b>Absensi</b>
          <span>Catat kehadiran dengan mudah.</span>
        </div>
        <div>
          <BookOpen />
          <b>Materi</b>
          <span>Materi harian selalu rapi.</span>
        </div>
        <div>
          <ClipboardCheck />
          <b>Tugas</b>
          <span>Belajar dan kumpulkan tugas.</span>
        </div>
        <div>
          <Megaphone />
          <b>Kabar</b>
          <span>Pengumuman dari guru.</span>
        </div>
      </section>
    ...
    <div className="overlay" onMouseDown={onClose}>
      <div className="login-modal" onMouseDown={(e) => e.stopPropagation()}>
        ...
        <input
          ...
          maxLength="8"
        />
        <button className="hero-button full" onClick={() => onLogin(name, pin)}>
...
function Dashboard({ user, data, go }) {
  const att =
    user.role === "teacher"
      ? data.attendance
      : data.attendance.filter((x) => x.Name === user.name);
  const present = att.filter(
    (x) => x.Status === "present" || x.Status === "Hadir"
  ).length;
  ...
  return (
    <>
      <Head
        ...
        text={
          user.role === "teacher"
            ? "Semoga kegiatan belajar hari ini berjalan menyenangkan."
      ...
      <div className="stats">
        {(user.role === "teacher"
          ? [
              [Users, data.students.length, "Murid", "students"],
              [CalendarCheck2, present, "Hadir", "attendance"],
              [BookOpen, data.materials.length, "Materi", "materials"],
              [ClipboardCheck, data.assignments.length, "Tugas", "assignments"],
            ]
          : [
              [CalendarCheck2, present, "Hadir", "attendance"],
              [BookOpen, data.materials.length, "Materi", "materials"],
              [ClipboardCheck, data.assignments.length, "Tugas", "assignments"],
              [Megaphone, data.announcements.length, "Pengumuman", "announcements"],
            ]
        ).map(([I, v, l, p]) => (
          <button onClick={() => go(p)} className="stat" key={l}>
            <I />
            <span>{l}</span>
            <b>{v}</b>
          </button>
      ...
      <div className="grid2">
        <section className="panel">
          <div className="panel-title">
            <div>
              ...
              <span>Belajar hari ini dengan santai.</span>
            </div>
            <button onClick={() => go("materials")}>Lihat →</button>
          </div>
        ...
        <section className="panel">
          <div className="panel-title">
            <div>
              <h2>Pengumuman</h2>
              <span>Pesan dari guru.</span>
            </div>
            <button onClick={() => go("announcements")}>Lihat →</button>
          </div>
          {data.announcements.slice(0, 3).map((m, i) => (
            <div className="row" key={i}>
              <div className="round-icon">
                <Megaphone size={17} />
              </div>
      ...
      {user.role === "student" && (
        <section className="panel form-panel">
          ...
          <div className="choice-grid">
            {[
              ["present", "Hadir", "Saya mengikuti belajar."],
              ["sick", "Sakit", "Saya sedang sakit."],
              ["permission", "Izin", "Saya berhalangan hadir."],
              ["absent", "Alpa", "Saya tidak hadir."],
            ].map((x) => (
              <button
                className={status === x[0] ? "choice active" : "choice"}
                onClick={() => setStatus(x[0])}
                key={x[0]}
              >
                <span className={"dot " + x[0]}></span>
                <b>{x[1]}</b>
                <small>{x[2]}</small>
              </button>
            ))}
          ...
          <textarea
            ...
            placeholder="Tulis keterangan bila perlu..."
            rows="3"
          />
          {status !== "present" && (
            <label className="photo-input">
              <Upload size={18} />
              <span>{photo ? photo.name : "Upload bukti foto (opsional)"}</span>
              <input
                type="file"
                accept="image/*"
          ...
          <button className="hero-button" onClick={submit}>
            <Send size={17} /> Simpan absensi
          </button>
          {sent && <div className="success">Absensi tersimpan 🌷</div>}
        </section>
      )}
      <section className="panel">
        <div className="panel-title">
          <div>
            <h2>Riwayat</h2>
            <span>Catatan kehadiran terbaru.</span>
          </div>
        </div>
        <div className="table">
          <div className="tr th">
          ...
          {mine.slice(0, 30).map((a, i) => (
            <div className="tr" key={i}>
              <span>{a.Name || a.name}</span>
              <span>{a.Date || a.date || "-"}</span>
              <span>
                <em className={"badge " + (a.Status || a.status)}>
...
function Materials({ user, data, reload }) {
  return (
    <CrudPage
      ...
      text="Materi dibuat rapi agar mudah dibaca kapan saja."
      icon={BookOpen}
...
function Assignments({ user, data, reload }) {
  return (
    <CrudPage
      type="assignment"
      user={user}
      ...
      title="Tugas"
      eyebrow="Pembelajaran"
      text="Sedikit latihan setiap hari membuatmu makin hebat."
      icon={ClipboardCheck}
      fields={["Title", "Instructions", "Due"]}
      ...
      {open && (
        <div className="overlay" onMouseDown={() => setOpen(false)}>
          <div className="login-modal wide" onMouseDown={(e) => e.stopPropagation()}>
            ...
            {fields.map((f) => (
              <div key={f}>
                ...
                {f === "Content" || f === "Instructions" ? (
                  ...
                  <input
                    type={f === "Date" ? "date" : f === "Due" ? "datetime-local" : "text"}
                    value={form[f] || ""}
                    onChange={(e) => setForm({ ...form, [f]: e.target.value })}
                  />
                )}
            ...
            <button className="hero-button full" onClick={save}>
              Simpan <Send size={16} />
            </button>
          </div>
        </div>
  ...
  return (
    <>
      <Head
        eyebrow="Kelas"
        title="Murid"
        text="Kursi kecil, perjalanan belajar yang berbeda."
      />
      <div className="student-grid">
        {data.students.map((s) => (
          <article className="student-card" key={s.Id}>
            <div className="avatar big">{s.Name[0]}</div>
            <div>
              <h2>{s.Name}</h2>
              <p>{s.Grade || "Murid SD"}</p>
              <span>PIN: ••••</span>
            </div>
          </article>
        ))}
...
function fileToBase64(file) {
  return new Promise((resolve, reject) => {
    ...
    r.onload = () => {
      const [head, data] = r.result.split(",");
