...
// 1. TAMBAHAN: Daftar Paket Bimbel
const PAKET_BIMBEL = [
  "Calistung 30 Menit (8x Pertemuan) - Rp 250.000",
  "Calistung 60 Menit (8x Pertemuan) - Rp 400.000",
  "Jarimatika 60 Menit (8x Pertemuan) - Rp 400.000"
];
...
function localAction(action, b) {
  ...
  // TAMBAHAN: Menyimpan Paket Siswa secara lokal (Demo)
  if (action === "updateStudent") {
    const s = demo.students.find(x => x.Name === b.name);
    if(s) { s.Paket = b.paket; s.StatusBayar = b.statusBayar; }
    return {ok: true};
  }
...
function App() {
  ...
  const login = async (name, pin) => {
    ...
    // 2. PERBAIKAN: Deteksi Guru lebih kuat
    if (
      nameStr === "guru" ||
  ...
  return (
    ...
    >
      ...
      {page === "students" && (
        <Students data={data} reload={load} />
      )}
    </Shell>
  );
}
...
function Shell({
  ...
  const [mobileMenu, setMobileMenu] = useState(false); // State untuk Menu HP
      ...
      <main className="content">
        ...
        {mobileMenu && (
          <div style={{
            ...
            background: 'rgba(255,250,247,0.98)', padding: '25px',
            display: 'flex', flexDirection: 'column',
            animation: 'pageEnter 0.2s ease', overflowY: 'auto'
          }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '30px' }}>
              <b style={{ fontSize: '22px', color: '#5a4e47', fontFamily: '"Playfair Display", serif' }}>Menu Utama</b>
              <button onClick={() => setMobileMenu(false)} style={{ background: '#f8efec', border: 'none', width: '40px', height: '40px', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <X size={22} color="#8b7e7a" />
              </button>
            </div>
              ...
              {desktopMenus.map(([id, label, Icon]) => (
                <button
                  ...
                  style={{
                    ...
                    background: page === id ? '#f8e9e5' : '#ffffff',
                    border: '1px solid #f0e6e0', borderRadius: '16px',
                    color: '#5a4e47', fontSize: '15px', fontWeight: page === id ? 'bold' : 'normal',
                    boxShadow: '0 4px 15px rgba(0,0,0,0.02)'
                  }}
                >
                  <Icon size={22} color={page === id ? "#a76366" : "#b4a5a0"} /> {label}
                </button>
              ))}
            </div>
            ...
            <button onClick={logout} style={{
              ...
              padding: '16px', background: '#fae5e5', color: '#9c6060', border: 'none', borderRadius: '16px', fontWeight: 'bold', fontSize: '15px'
            }}>
          ...
          {user.role === "student" && (
            <div className="profile-details">
              ...
              <div>
                ...
                <b style={{ color: user.StatusBayar === 'Lunas' ? '#75a46f' : '#d67878' }}>
                  {user.StatusBayar || "Belum Lunas"}
                </b>
              </div>
            </div>
          )}
...
/* =========================================================
   STUDENTS (DENGAN MANAJEMEN PAKET UNTUK GURU)
========================================================= */

function Students({ data, reload }) {
  const [selectedStudent, setSelectedStudent] = useState(null);
  const [form, setForm] = useState({ paket: "", statusBayar: "" });
  const [loading, setLoading] = useState(false);
  ...
  return (
    <div className="page-enter">
          ...
          .map(s => (
            <article
              ...
              onClick={() => { 
                ...
                setForm({paket: s.Paket||"", statusBayar: s.StatusBayar||""}); 
              }}
            ...
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px', width: '100%' }}>
                ...
                <div>
                  <h2 style={{ fontSize: '18px', margin: 0 }}>{s.Name}</h2>
                  <p style={{ margin: '0 0 4px 0' }}>{s.Grade || "Murid SD"}</p>
              ...
              <div style={{ marginTop: '5px', background: '#fcfaf8', width: '100%', padding: '12px', borderRadius: '12px', border: '1px solid #f0e6e0' }}>
                <div style={{ fontSize: '9px', color: '#a1938f', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Paket Bimbel</div>
                <div style={{ fontSize: '11px', fontWeight: 'bold', color: '#5a4e47', marginBottom: '8px' }}>{s.Paket || "Belum dipilih"}</div>
                
                <div style={{ fontSize: '9px', color: '#a1938f', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Status Bayar</div>
                <div style={{ fontSize: '11px', fontWeight: 'bold', color: s.StatusBayar === 'Lunas' ? '#75a46f' : '#d67878' }}>{s.StatusBayar || "Belum Lunas"}</div>
              </div>
            </article>
          ))}
      </div>
      ...
      {selectedStudent && (
        <div className="overlay" onMouseDown={() => !loading && setSelectedStudent(null)}>
          <div className="login-modal wide" onMouseDown={e => e.stopPropagation()}>
            <button className="modal-close" onClick={() => !loading && setSelectedStudent(null)}>
              <X size={18} />
            </button>
            <div className="modal-flower">✿</div>
            <h2>Update Data {selectedStudent.Name}</h2>
            ...
            >
              ...
              {PAKET_BIMBEL.map(p => <option key={p} value={p}>{p}</option>)}
            </select>
            
            <label>Status Pembayaran</label>
            <select 
              value={form.statusBayar} 
              onChange={e => setForm({...form, statusBayar: e.target.value})}
            >
              <option value="Belum Lunas">Belum Lunas</option>
              <option value="Lunas">Lunas</option>
            </select>

            <button className="hero-button full" onClick={save} disabled={loading}>
